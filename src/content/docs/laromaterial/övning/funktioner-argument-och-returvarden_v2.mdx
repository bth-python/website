---
title: Funktioner, argument och returvärden
description: "Lär dig om funktioner och dess beståndsdelar"
---
...
### Fördefinierade värde (default value)
...
> Emil och Andreas kollegor Mikael och Kenneth köper mat i restaurangen. Men hissen upp till matsalen är långsam så deras mat hinner kallna innan de är uppe. Båda värmer maten, men bara på 600W i 30 sek.
...
## Formatering för bättre utskrifter

Vi har ett fungerande program men det finns nästan alltid små detaljer vi kan förbättra. I detta fallet tycker jag att utskrifterna blir tråkiga med alla dessa decimaler, så låt oss titta på ett sätt att få bättre utskrifter från våra program i Python. Vi ska använda f-strängar (*formatted string literal*). Vi skriver f innan själva strängen som omsluts av "" och använder `{}` för de värden vi vill ersätta med de vi har beräknat. Inom `{}` anger vi vilka värden vi vill visa som i detta fall motsvaras av innehåller i variabler, men det kan även vara t.ex beräkningar som resulterar i ett resultat.
...
nice_string = f"Emil använder {emil_energy} kWh och detta kostar {emil_cost} kr"
# skriver ut:
# Emil använder 0.033333333333333326 kWh och detta kostar 0.02601333333333333 kr
```
