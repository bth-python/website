...
Vi fortsätter på exemplet från övningen [Funktioner, argument och returvärden](funktioner-argument-och-returvarden) så har du inte gjort den övningen rekommenderar vi att du gör den först.
...
I slutet av övningen [Funktioner, argument och returvärden](funktioner-argument-och-returvarden) ser vår funktion ut på följande sätt.
...
Alla kodexempel från denna övningen finns går att hitta [här](https://github.com/dbwebb-se/python/tree/master/example/functions). Kodexemplet innehåller hela exemplet från tidigare övningar och versioner, men uppdelat i modulen `sandwich.py` och huvudprogrammet finns i `waysub.py`.
